...
export async function finishAttempt(
  attemptId: string,
  answers: { questionId: string; selectedIndex: number | null; selectedOption?: string | null }[]
) {
  const attempt = await Attempt.findById(attemptId);
  if (!attempt) {
    throw new ApiError("Attempt not found", 404);
  ...
  const details = answers.map((answer) => {
    ...
    const selectedOption =
      typeof answer.selectedOption === "string" ? answer.selectedOption.trim() : null;
    const isCorrect = selectedOption && correctOption
      ? selectedOption === correctOption
      : answer.selectedIndex !== null && answer.selectedIndex === correctIndex;
