...
export default function StudentsPage({ params }: { params: { quizId: string } }) {
  ...
  async function upload() {
    ...
    const res = await fetch(`/api/quizzes/${params.quizId}/students/upload`, {
      method: "POST",
    ...
    setUploading(false);
    if (!res.ok) {
      setMessage(data?.error || "Upload failed. Use CSV/XLSX with headers: Name,Email,StudentId.");
      return;
    }
  ...
  async function deleteStudent(studentId: string) {
    ...
    await fetch(`/api/quizzes/${params.quizId}/students/${studentId}`, { method: "DELETE" });
    await loadStudents();
  }
  ...
  async function saveEmail(studentId: string) {
    ...
    const res = await fetch(`/api/quizzes/${params.quizId}/students/${studentId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
