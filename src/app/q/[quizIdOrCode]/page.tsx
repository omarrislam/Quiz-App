...
export default function QuizPage({ params }: { params: { quizIdOrCode: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/quizzes/${params.quizIdOrCode}/public`)
      .then((res) => res.json())
      .then((data) => {
        if (data?.title) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/quizzes/${params.quizIdOrCode}/students/public?q=${encodeURIComponent(name)}`, {
  ...
  async function verify() {
    setError("");
    setMessage("");
    const res = await fetch(`/api/quizzes/${params.quizIdOrCode}/verify-otp`, {
      method: "POST",
  ...
  async function resendOtp() {
    setMessage("");
    setError("");
    const res = await fetch(`/api/quizzes/${params.quizIdOrCode}/invitations/resend-otp`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
